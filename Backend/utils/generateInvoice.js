...
const generateInvoice = (order) => {
  return new Promise((resolve, reject) => {
    ...
    if (
      order.sampleBox &&
      order.sampleBox.length > 0 &&
      order.sampleBox[0].items.length > 0
    ) {
      y += 20;

      doc.fontSize(12).fillColor("#6A0DAD").text("Sample Box", 40, y);

      y += 20;

      doc.fontSize(10).fillColor("black");

      order.sampleBox[0].items.forEach((sample) => {
        doc.text(`• ${sample}`, 60, y);
        y += 18;
      });
      ...
      doc.text(
        `Sample Box Price : Rs. ${order.sampleBox[0].totalPrice}`,
        40,
        y,
      );
    ...
    const samplePrice = order.sampleBox?.[0]?.totalPrice || 0;
    ...
    // Sample Box
    y += 15;
    doc.text(`Sample Box : Rs. ${samplePrice}`, 350, y);
